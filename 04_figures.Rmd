...
(ref:figure2)  Image of the F/V `r boat`  used for the `r yr` Sablefish research and assessment survey.  Photo credit: Cody Melnychuk. 

```{r figure2, fig.cap='(ref:figure2)', out.width = "480px", out.height = "480px",echo=FALSE, fig.align="center", warning=FALSE}
...
(ref:figure3) Trap gear elements consisting of 25 baited traps snapped to beckets along a groundline (A). Trap and camera trap elements (B).  
...
(ref:figure4) Start locations of survey sets (red markers) conducted in `r yr` for the stratified random survey areas S~1~ through S~5~.  Movement/deep-water autonomous camera locations (triangle symbols) are labelled in red font.

```{r figure4, fig.cap='(ref:figure4)', out.width = "450px",  fig.align="center", warning=FALSE, echo=FALSE, message=FALSE, error=FALSE}
      img <- paste(basepath,'figures/figure4.png',sep='') 
      knitr::include_graphics(img)
```
\clearpage

(ref:figure5) Location of the `r yr` standardized sets within the Dean/Burke Channel mainland inlet locality.  

```{r figure5, fig.cap='(ref:figure5)', out.width = "450px", echo=FALSE, fig.align="center", warning=FALSE}
      img <- paste(basepath,'figures/Figure5.png'  ,sep="")
      knitr::include_graphics(img)
```
\clearpage

(ref:figure6) Map of allocated vs completed survey blocks for the 2021 survey sets.  Star symbols depict rationale for dropped survey blocks.

```{r figure6, fig.cap='(ref:figure6)', out.width = "450px",  fig.align="center", warning=FALSE, echo=FALSE, message=FALSE, error=FALSE}
      img <- paste(basepath,'figures/figure6.png',sep='') 
      knitr::include_graphics(img)
```
\clearpage

(ref:figure7) Average Sablefish catch per unit effort (CPUE; mean +/- 95% CIs) by survey strata since 2003. Panels run deep to shallow (left to right) and north to south (top to bottom).

```{r figure7, fig.cap='(ref:figure7)', out.width = "450px",  fig.align="center", warning=FALSE, echo=FALSE, message=FALSE, error=FALSE}
    ...
    tgc.02            <- arrange(transform(tgc01, 
     ...
     tgc.02$col <- ifelse(tgc.02$SET_YEAR == 2021, "red", "steelblue")

     ggplot(tgc.02,aes(x=SET_YEAR,y=sable_cpue,label = round(sable_cpue,1))) +
            #geom_text(size = 2.0, vjust = 2.9, nudge_y = 8.5) + 
...
```{r figure8, fig.cap='(ref:figure8)', out.width = "450px", out.height = "576px",echo=FALSE, fig.align="center", warning=FALSE}
    ...
    tgcnt$col <- ifelse(tgcnt$SET_YEAR == 2021, "red", "steelblue")
    
    # print(tgcnt[tgcnt$SET_YEAR >=2021,])
    
    ggplot(tgcnt, aes(x=SET_YEAR,y=sable_cnt,label = round(sable_cnt,1))) +   # add label temporary visual
           #geom_text(size = 4.0, vjust = 2.9, nudge_y = 8.5) +    # add text just for temporary visual
...
```{r figure9, fig.cap='(ref:figure9)', out.width = "450px", out.height = "576px",echo=FALSE, fig.align="center", warning=FALSE}
    ...
    png(paste(basepath,'figures/figure9.png',sep=''),width=500, height=641) # write png to file
    par(mfcol=c(1,1))
    tgcwg$col <- ifelse(tgcwg$SET_YEAR == 2021, "red", "steelblue")    
    

    ggplot(tgcwg, aes(x=SET_YEAR,y=sable_wg,label = round(sable_wg,0))) +   # add label just for temporary visual
           #geom_text(size = 2.7, vjust = 0.5, nudge_y = 1.5) +    # add text just for temporary visual
...
(ref:figure10) (A) Annual mean weight of Sablefish per trap (kg/trap); (B) annual mean number of Sablefish per trap (#fish/trap); (C) annual mean weight of Sablefish (kg) by StRS survey strata over time. Horizontal line is median and blue dots are arithmetic mean.

```{r figure10, fig.cap='(ref:figure10)', out.width = "450px", out.height = "576px",echo=FALSE, fig.align="center", warning=FALSE}

  png(paste(basepath,'figures/figure10.png', sep=''), width=600, height=800) # write png to file
  library(cowplot)
...
(ref:figure11) Annual distributions of catch statistics over all mainland inlet indexing sets between 1994 and 2019, including: (A) CPUE in units of weight of Sablefish per trap (kg/trap);  (B) CPUE in units of Sablefish per trap (#fish/trap); and (C) mean Sablefish body weight (kg). Horizontal line is median, grey shading shows the 25th and 75% percentiles, and blue dots show arithmetic means. No inlets were surveyed in 2020.  Dean/Burke Channel inlet was the only inlet surveyed in 2021 and not included in these figures.

```{r figure11, fig.cap='(ref:figure11)', results='asis', out.width = "450px",out.height = "560px", fig.align="center"}

    png(paste(basepath,'figures/figure11.png',sep=''), width=600, height=800) # write png to file
    cpuedt    <-  "SELECT  * from dbo.GENERIC_GFBIO_TRAPS where year < 2022 and [Spatial.Stratum] not like '%Quatsino Sound%' "
    ...
    cpuedat.Inlet   <-  read.csv(paste(path,'figure11.csv',sep=''),header=T)
    ...
    inlet       <- dat.Inlet[dat.Inlet$year >1993 & dat.Inlet$year <= 2019,]   
    ...
    inlet.2wg         <-  arrange(transform(inlet[inlet$year>1993,],year=factor(year,levels=c(1994,1995,1996,1997,
     ...
     p <- ggplot(inlet.2w, aes(x=year, y=CPUE.SABLE.WEIGHT/CPUE.TRAPS)) +
          ...
          scale_x_discrete(breaks = seq(1994, 2019, by = 2)) +
             annotate(geom="text", x=22, y=95, label="Mainland inlets",
                 color="steel blue", size=5) +
     ...
     while (!is.null(dev.list()))  dev.off()
     img <-  paste(paste(basepath,'figures/figure11.png',sep=""))   # -- retrieve png 
          knitr::include_graphics(img)

```
\clearpage

(ref:figure12) Length frequencies for female (grey) and male Sablefish (steel blue) up to `r yr` for all StRS sets.  Specimen number (n), mean ($\overline{x}$) and standard deviation ($\sigma$) are displayed.

```{r figure12, fig.cap='(ref:figure12)',results='asis',out.width = "420px",out.height = "245px", fig.align="center"}
knitr::opts_chunk$set(out.height = "\\textheight",  out.width = "\\textwidth")
      ...
      png(paste(basepath,'figures/figure12.png', sep=''), width = 800, height = 430)
      ...
      par(xpd=NA)  # for placement of A,B
    	...
    	plot(b2, col=rgb(0.27,.51,.71,0.5), border="white", main="", xlab="Length (cm)", xlim=c(30,100), cex=1.4)
    	plot(a2, col=rgb(0.1,0.1,0.1,0.5),  border="white", add = TRUE, cex=1.4)
    	box()
    	
      panLab(0.79,0.4,paste("StRS 2003-2021",  sep=""), cex=1.4)  # -- Labels
    	panLab(0.19,0.97,"Sablefish males", cex=1.4)
    	panLab(0.79,0.97,"Sablefish females", cex=1.4)
    	
    	tf  <- prettyNum(tf, big.mark = ",", scientific = FALSE)
    	tm  <- prettyNum(tm, big.mark = ",", scientific = FALSE)
    	
    	panLab(0.19,0.9,bquote(bold(n)==.(tm)), cex=1.4)
    	panLab(0.79,0.9,bquote(bold(n)==.(tf)), cex=1.4)
    	
    	panLab(0.19,0.8,bquote(bolditalic(bar(x))==.(mLm)), cex=1.4)
    	panLab(0.79,0.8,bquote(bolditalic(bar(x))==.(mLf)), cex=1.4)
    	
    	panLab(0.19,0.7,bquote(bold(sigma)==.(stdm)), cex=1.4)
    	panLab(0.79,0.7,bquote(bold(sigma)==.(stdf)), cex=1.4)
...
```{r figure13, fig.cap='(ref:figure13)',results='asis', out.width = "450px",out.height = "240px", fig.align="center"}

     png(paste(basepath,'figures/figure13.png',sep=''), width = 900, height = 530) # -- starts writing a png to figure folder
     ...
     plot(fdat$YEAR, fdat$AvL,pch=2, ylim=c(50,MxSeam+5),xlim=c(2003,yr+1), cex=1.3, 
          ...
          col="gray30",xaxt="n")
     axis(1, seq(2003,2021,1), cex.axis=1.3)
     ...
     text(fdat$YEAR, fdat$AvL+1.5, fdat$count,cex=1.4)
     text(mdat$YEAR, mdat$AvL+1.5, mdat$count,cex=1.4)
     ...
     legend(0.035,1, c("  Sablefish females","  Sablefish males"), 
           lty=c(2,1), lwd=2, bty="n", col=c("gray50","steelblue"), cex=1.4)
...
```{r figure14, fig.cap='(ref:figure14)',results='asis', out.width = "410px", out.height = "235px", fig.align="center"}
...
```{r figure15, fig.cap='(ref:figure15)',results='asis', out.width = "350px",out.height = "235px", fig.align="center"}
  ...
  polarsumm <-   paste( " select YEAR, SABLE_AREA_GROUP, SABLE_DEPTH_GROUP from ",
   ...
   sublabel    <- (c("S1S","S1M","S1D",
    ...
    polar.dat  <-  polar[polar$YEAR>=2016,]
  ...
  while (!is.null(dev.list()))  dev.off()  
    img <-   paste(basepath,'figures/figure15i.png',sep='')   # retrieve png from figures folder
             knitr::include_graphics(img)
```
\clearpage
...
```{r figure18, fig.cap='(ref:figure18)', results='asis', out.width = "480px",out.height = "616px", fig.align = "center"}
      ...
      #write.table(sea.bird.ridge, file = paste(path,"figure18.csv",sep=''),row.names=FALSE, na="",col.names=TRUE,  sep=",") 
