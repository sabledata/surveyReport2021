...
```{r appendixC, results="asis"}
   ...
   names(trap.set)  <-  c("surveyset","Type")
    ...
    trap.set$get.color[trap.set$Type=='Movement']   <- "lightskyblue"   # 60 trap sets
    trap.set$get.color[trap.set$surveyset 
                       %in% c(5,6,11,12,13,19,20,25,26,31,32,33,38,43,44,45,49,51,62,67,68,69,70)] <- "plum2"  # 25 trap sets
    # tail(trap.set)
...
```{r appendixESetUp, warning=FALSE, echo=FALSE, message=FALSE, error=FALSE}
  ...
  trap.count <-paste(
   ...
   trap.sable25 <-  trap.sable1[!trap.sable1$SET_ID %in% c(7,14,27,63),]  # 60 TRAP SETS SO HUGE THEY NOT SHOWN
   ...
   # note 2021, all sets picked up from the end
   trap.sable.end   <- trap.sable
   ...
   w<-trap.sable.end %>%   # create sparkline graphs
     ...
     do({
       p <- ggplot(., aes(x = Trap, y = Value)) + 
            ...
            geom_line(linewidth = 3, color="dark green") +      
...
```{r appendixG, echo=FALSE}
   ...
   colnames(otherspec2)   <-  c("Species Name", "Set", "Fork Length",
                               "Weight", "Sex","Maturity", "Aging Structures Collected", "DNA",
                               "Total", 
                               "Prop Males", 
                               "Males", "Females",  
                               "No sex" )
