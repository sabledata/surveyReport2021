...
# SABLEFISH CHARTER SET LOG  `r yr`. {#app:second-appendix}
Example of the set log data form with directions from the survey instruction manual.

```{r appendixB,out.width = "460px", out.height = "466px", fig.align="left"}

      if(knitr:::is_latex_output())
         # paper bridge log
         img <-  paste(basepath,'figures/AppendixB.png',sep="")
...
```{r appendixC, results="asis"}
   ...
   # no ctd or dissolved oxygen sensor
   srvy.set  <-   srvy.set[,c(-23,-25)]  
   ...
   srvy.set$Type[srvy.set$Set==7]  <-  'Movement'
   srvy.set$Type[srvy.set$Set==14] <-  'Movement'  
   srvy.set$Type[srvy.set$Set==27] <-  'Movement'
   srvy.set$Type[srvy.set$Set==63] <-  'Movement'  
   ...
   names(trap.set)  <-  c("surveyset","Type")
    ...
    trap.set$get.color[trap.set$Type=='Movement']   <- "lightskyblue"   # 25 trap sets
    trap.set$get.color[trap.set$surveyset %in% c(5,6,11,12,13,19,20,25,26,31,32,33,38,43,44,45,49,51,62,67,68,69,70)] <- "plum2"  # 60 trap sets
    # tail(trap.set)
...
```{r appendixESetUp, warning=FALSE, echo=FALSE, message=FALSE, error=FALSE}
   ...
   trap.sable.start <- trap.sable[trap.sable$PU =='START',]
   trap.sable.end   <- trap.sable[trap.sable$PU =='END',]
    ...
    w<-trap.sable.end %>%   # create sparkline graphs
     ...
     do({
       p <- ggplot(., aes(x = Trap, y = Value)) + 
            ...
            #geom_area( fill=trap.sable$col, alpha=0.4) + 
            geom_line(size = 4, color="dark green") +      
            #geom_point(size=15, color="red") + 
            ylim(0, big.value) +
            xlim(0,25) +
            theme_void() 
...
```{r appendixE, echo=FALSE}
   ...
   addition.cam                     <-  c("Movement", "", surveycam.Total.Count, "", 0, 
                                           ...
                                           0, 0, "", surveycam.Fork.Len.Sample.Count,
   ...
   surveyspec  <- surveyspec %>%
                  mutate(Sparkline2 = paste0("\\raisebox{-.28\\height} {\\includegraphics[width=0.8cm]{sets_",SET,".png}}") ) 
...
# SUMMARY OF BIOLOGICAL DATA FOR THE ROUGHEYE/BLACKSPOTTED ROCKFISH COMPLEX. {#app:sixth-appendix} 
...
```{r appendixF, echo=FALSE}
   ...
   othersamplesre <-  paste( "select species_name, [SET], [Len Sample Count], ",
                           ...
                           ") and (dbo.GFBIO_RESEARCH_SAMPLE_DETAILS_OTHER_FISH.species = N'394') order by species,[SET]", sep="")
   # otherspecre      <-  GetSQLData(othersamplesre,"Sablefish")   # read from SQL Server
   # write.table(otherspecre, file = paste(path,'appendixF.csv',sep=''),row.names=FALSE, na="",col.names=TRUE,  sep=",")   
   otherspecre       <-  read.csv(paste(path,'appendixF.csv',sep=''),header=T) # read from csv
   otherspecre[2,14] <- 0
   otherspecre[2,15] <- 0
   otherspecre[2,16] <- 0
   ...
   kableExtra::kable(otherspecre,
                     ...
                     longtable = T,
                     linesep   = "",
                     align=c('r','c','c','c','c','c','c','c','c','c','c','c','c','c','c'),
                     format = "latex") %>%
...
# SUMMARY OF BIOLOGICAL DATA FOR OTHER ROCKFISH SPECIES AND LINGCOD. {#app:seventh-appendix} 
...
```{r appendixG, echo=FALSE}
   ...
   #write.table(otherspec2, file = paste(path,'appendixG.csv',sep=''),row.names=FALSE, na="",col.names=TRUE,  sep=",")   
   otherspec2              <-  read.csv(paste(path,'appendixG.csv',sep=''),header=T) # read from csv
   ...
   colnames(otherspec2)   <-  c("Species Name", "Set", "Fork Length",
      ...
      kableExtra::kable(otherspec2,
                     ...
                     longtable = T,
                     linesep   = "",
                     align=c('l','r','c','c','c','c','c','c','c','c','c','c'),
                     format = "latex") %>%
      ...
      column_spec(8,width     = "0.2cm") %>% # tot cnt        
      column_spec(9,width     = "1.0cm") %>% # prop m
      column_spec(10,width    = "0.5cm") %>% # male
      column_spec(11,width    = "0.7cm") %>% # female
      column_spec(12,width    = "0.6cm") %>% #  no sex  
      row_spec(0, bold = T) %>%
