...
```{r CatchRateText, echo=FALSE}
  ...
  for (i in 1:nyrs)
    ...
    #In the deep waters (RD~3~), 2021 kg/trap was down from 2020 in South West Coast Haida Gwaii (S~4~) 
    #and South West Coast Vancouver Island (S~1~).  In the middle depth waters (RD~2~),  2021 kg/trap was down 
    #from 2020 in Queen Charlotte Sound (S~3~).  In the shallow waters (RD~1~),  2021 kg/trap was down from 2020 
    #in North West Coast Vancouver Island (S~2~) and South West Coast Vancouver Island (S~1~).  (FIGURE 7)
  
```
## CATCH PER UNIT EFFORT (CPUE) 

Catch per unit effort (CPUE) statistics for 2021 are presented in relation to the available time series for each of the survey components used to index abundance <!-- KH: added 'used to index abundance'-->:  (i) StRS (2003-2021) and (ii) traditional mainland inlet indexing sites (1991-2019). 

### Stratified Random Set CPUE 

Catch rates (catch per unit effort; CPUE) as indexed by kilograms of sablefish per trap (Figure \@ref(fig:figure7)) and numbers of fish per trap (Figure \@ref(fig:figure8)) were generally higher in the middle depth strata over the survey time series (2003-2021).   In 2020 and 2021, the kg/trap and #fish/trap in the middle depth strata (RD~2~) were lower than the peak reached in 2019 in all areas with the exception of the South West Vancouver Island (S~1~).

The mean weight <!--of captured sablefish -->in 2021 was similar or slightly lower compared to 2020 in areas S~2~, S~3~ and S~4~, and slightly higher in the areas S~1~ and S~5~ (Figure \@ref(fig:figure9)).   The stratified mean survey abundance in 2021 was 36 kg/trap, which is up 3% from 2020 and down -6% from the 2019-2020 average (Figure \@ref(fig:figure10)).

### Mainland Inlet CPUE

CPUE in the mainlaind inlets has varied <!--KH: suggestion to delete "in a relatively predictable manner"--> over time with peak CPUE occurring every 5-8 years (Figure \@ref(fig:figure11) a,b). In the early part of the time series (mid-1990s) average CPUE remained relatively constant before a peak in CPUE was observed in 1999, followed by declines to consistent levels until another peak in 2003 and 2004, and again in 2011. In 2018, CPUE returned (~23 fish, and ~40 kg per trap) to the levels observed during previous peaks. In 2019, the highest catch rates of the 26 year time series were observed (~36 fish, and ~61 kg per trap). Notably, the 2018-2019 mean weight (~1.7 kg) declined to an all time low due to large number of small fish becoming available to the survey (Figure \@ref(fig:figure11) c).  
...
```{r SpeciesCompositionText, echo=FALSE}
  ...
  # -- taxonomic top 5 group count StRS
   ...
   topStRSsp1   <- gsub("pacific","Pacific", topStRSsp1)            # -- capital on Pacific if it occurs
   ...
   topStRSsp2   <- gsub("pacific","Pacific", topStRSsp2)            # -- capital on Pacific if it occurs   
   topStRSsc2   <- firstup(tolower(as.character(topStRSsp[2,2])))   
   
   topStRSsp3   <- tolower(as.character(topStRSsp[3,1]))
   topStRSsp3   <- gsub("pacific","Pacific", topStRSsp3)            # -- capital on Pacific if it occurs   
   topStRSsc3   <- firstup(tolower(as.character(topStRSsp[3,2])))
   
   topStRSsp4   <- tolower(as.character(topStRSsp[4,1]) ) 
   topStRSsp4   <- gsub("pacific","Pacific", topStRSsp4)            # -- capital on Pacific if it occurs   
   topStRSsc4   <- firstup(tolower(as.character(topStRSsp[4,2])))  
   
   topStRSsp5   <- tolower(as.character(topStRSsp[5,1]))  
   topStRSsp5   <- gsub("pacific","Pacific", topStRSsp5)            # -- capital on Pacific if it occurs   
  ...
  # -- taxonomic group count Inlets
   ...
   topInletsp1  <- gsub("pacific","Pacific", topInletsp1)          # -- capital on Pacific if it occurs
...
```{r SamplingWords, echo=FALSE}
   ...
   # -- other fish counts
     ...
     tagSample  <-  paste(" select TAG_ID, RELEASE_YR, RECOVERY_YR, RELEASE_TAG, RECOVERY_TAG, ",
                          ...
                          " where (RECOVERY_YR = 2021) and (RECOVERY_STATE_DESC='fresh') ",
...
```{r basketFateWords, echo=FALSE}
   ...
   trap.dat    <- read.csv(paste(path,'appendixD.csv',sep=''),header=T)
...
Differences in length distributions between female and male sablefish are exhibited in the data collected from the StRS portion of the 2003 - `r yr` surveys. Over these `r yr-2003 +1` years, the mean fork length ($\bar{x}$)  was `r mLf` cm for females and `r mLm` cm for males (Figure \@ref(fig:figure12)).

In `r yr`, the average mean fork length for the `r format(length(firstyrfem$sex), big.mark=",")` females was `r as.character(round(mean(firstyrfem$length),1))` cm  and the average mean fork length for the `r format(length(firstyrmal$sex), big.mark=",")` males was `r as.character(round(mean(firstyrmal$length),1))` cm.  The average length of males reached their lowest mean size since 2003 (Figure \@ref(fig:figure13)). 
...
More than half of the sub-legal specimens were captured in the mid-depth waters of i) the northern strata of S~4~ and S~5~ in 2017 and 2018, ii) all spatial strata in 2019, iii) S~1~, S~4~ and S~5~ in 2020, and iv) S~1~, S~2~, S~4~ and S~5~ in 2021 (Figure \@ref(fig:figure15)).
...
```{r OtherFishText, echo=FALSE}
   ...
   otherspecRE  <-  read.csv(paste(path,'appendixF.csv',sep=''),header=T) # read from appendix F, rougheye biosamples
 ...
 #  if (hy.count==0) {hy.count ='none'}
   ...
   otherspec  <-  read.csv(paste(path,'appendixG.csv',sep=''),header=T) # read from appendix G biosamples
   ...
   total <- ""
    for (i in c(len.sp.cnt:1)) {
        if (i==len.sp.cnt-1){
            addition <- paste(tolower(species.names[i]),", and ", sep="")}  # add 'and'
        else { 
            addition <- paste(tolower(species.names[i]),", ", sep="")}  # add comma between species
...
Length, sex, maturity, otoliths and DNA samples were collected for `r rebs.count` rougheye/blackspotted rockfish samples.  The science samplers visually identified `r re.count` specimens as rougheye, `r bs.count` specimens as blackspotted and `r hy.count` specimen as a hybrid species (Appendix \@ref(app:sixth-appendix)).
...
respectively. Then, another cohort appeared in 2011 through to 2015, showing up as 3, 4, 5, 6 and 7 year olds. In 2016, 2017 and 2018 the highest proportion of female sablefish were ages 3,
4, and 5 year olds.  Last, the years 2019, 2020 and 2021 were represented <!--KH: replace 'represented' with 'dominated' --> by 3, 4 and 5 year old female sablefish, respectively (Figure \@ref(fig:figure16) a). <!--KH addition: This pattern suggests a large 2016 recruitment event with fish from this cohort dominating StRS catch in recent years -->

The highest proportion of male ages in StRS sets for 2003 through to 2011 were 3, 5, 5, 6, 8, 8, 8, 10 and 12 years of age, respectively. Another cohort appeared in 2012 through to 2016 as 4,
5, 7, 7 and 8 year olds. <!--KH: suggestion to replace last sentence with this: Another cohort dominated StRS catch starting in 2012, appearing first as 4 year olds in 2012, folled by 5 year olds in 2013 and 6 year olds in 2014--> <!--My reason for this last suggestion is that the repeat of age 7 twice in a row is a bit confusing.  Not clear if this is aging error??  -->A cohort began to apprear in 2017, with that year which was dominated by 3 year olds and in 2018 by 5 year olds.  <!--KH: suggest removing the last sentence about the 2017 appearance because the age 3 followed by age 5 is confusing -->The years 2019, 2020 and 2021 were represented by 3, 4 and 5 year old males, respectively (Figure \@ref(fig:figure16) b) <!--KH suggestion to add: ,as was seen for females -->.
...
A special thanks to the Vessel Master and crew of the F/V `r boat`, whose efforts made the survey successful and safe during the COVID-19 pandemic.  In `r yr`, the crew consisted of `r credits[1,4]`.
