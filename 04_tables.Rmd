...
```{r table1, echo=FALSE}
       ...
       Strata      <- c(paste( "S", "\\textsubscript{1}", " (South West Coast Vancouver Island or SWCVI)", sep=''),
                        ...
                        paste( "S", "\\textsubscript{3}", " (Queen Charlotte Sound or QCS)", sep=''),                        
                        paste( "S", "\\textsubscript{4}", " (South West Coast Haida Gwaii or SWCHG)", sep=''),                       
                        paste( "S", "\\textsubscript{5}", " (North West Coast Haida Gwaii or NWCHG)", sep=''),                       
                               "Total ")
...
```{r table2,  echo=FALSE}
    # refer to procedure exec dbo.Build_Seabird_tables at end for counts 
    # SELECT SPATIAL_STRATUM, DEPTH_STRATUM, YEAR, COUNT(FE_MAJOR_LEVEL_ID) AS CountSets
    # FROM  (SELECT SENS.YEAR, SENS.TRIP_ID, SENS.FE_MAJOR_LEVEL_ID, SENS.SBE, SENS.ACC, SENS.CTD, SENS.CAM, SENS.DO, 
    ...
    # HAVING (dbo.SURVEY_trips_vw.YEAR = 2021)) AS SENS INNER JOIN
    ...
    # WHERE (SENS.CAM = 1) AND (NOT (SENS.FE_MAJOR_LEVEL_ID IN (7, 14, 27, 63)))) AS SENS2
    ...
    Strata    <-   c(paste( "S", "\\textsubscript{1}", " (SWCVI)", sep='') , 
                     ...
                     paste( "S", "\\textsubscript{5}", " (NWCHG)", sep='') ,
                            "Inlet (Dean/Burke)")
    Fm100250  <-   c(2,2,2,2,2,0)
    RD1       <-   c(3,3,1,2,0,0)
    Fm250450  <-   c(2,2,2,2,2,0)
    RD2       <-   c(4,4,1,2,0,0)
    Fm450750  <-   c(1,1,1,1,1,0)
    RD3       <-   c(0,1,0,0,1,0)
    Inlet     <-   c(0,0,0,0,0,1)
    Total     <-   c(5,5,5,5,5,0)  
    
    Total2021 <-   c(RD1[1] + RD2[1] + RD3[1], RD1[2] + RD2[2] + RD3[2],
                     RD1[3] + RD2[3] + RD3[3], RD1[4] + RD2[4] + RD3[4],
                     RD1[5] + RD2[5] + RD3[5], 1)
    
    datastrata<-   data.frame(Strata, Fm100250, RD1, Fm250450,RD2, Fm450750,RD3,Inlet,Total,Total2021)    
    ...
    LastLine  <-   c('', sum(datastrata$Fm100250), sum(datastrata$RD1),
                     ...
                     sum(datastrata$Inlet),
                     sum(datastrata$Total) ,sum(datastrata$Total2021))
   
    datastrata <- rbind(datastrata, LastLine)
    datastrata$Strata <- as.character(datastrata$Strata)
    datastrata$Strata[7]  <- "Total"
    ...
    datastrata$Inlet   <-as.character(datastrata$Inlet)
    datastrata$Total   <-as.character(datastrata$Total)     
    datastrata$Fm100250[6]<-" "
    datastrata$Fm250450[6]<-" "
    datastrata$Fm450750[6]<-" "
    datastrata$RD1[6]<-" "
    datastrata$RD2[6]<-" "
    datastrata$RD3[6]<-" "    
    datastrata$Total[6]<-" "   
    datastrata$Inlet[c(1,2,3,4,5)]<-" " 
    
    colnames(datastrata) <- c("Strata", paste("RD", "\\textsubscript{1}", sep='') , paste("RD", "\\textsubscript{1}"," 2021", sep=''), 
                                        paste("RD", "\\textsubscript{2}", sep='') , paste("RD", "\\textsubscript{2}"," 2021", sep=''),
                                        paste("RD", "\\textsubscript{3}", sep='') , paste("RD", "\\textsubscript{3}"," 2021", sep=''), 
                                        "Inlet", "Total", "Total 2021") 
       ...
       kableExtra::kable(datastrata, booktabs = TRUE,
              ...
              column_spec(8, width  = "0.7cm", color="blue") %>%          
              column_spec(9, width  = "0.7cm") %>%
              column_spec(10,width  = "0.7cm", color="blue") %>%
              row_spec(0, color="black") %>%
              row_spec(7, bold=T) %>%
...
```
\ \
(ref:table3Caption) Details of completed 60 trap camera movement sets. Seabird temperature and pressure recorder (SBE39), Hobo accelerometer (HOBO) and camera (CAM) are indicated with an 'x'.
...
```{r table3,  echo=FALSE}
  ...
  trap60 <- paste("SELECT YEAR, TRIP_ID, set_no, becket_no, trap_no, MAX(SBE39) AS SBE39, MAX(HOBO) AS HOBO,  ",
                  ...
                  " FE.FE_MINOR_LEVEL_ID AS trap_no, CASE WHEN FE_EVENT_SENSOR_DESC LIKE '%sbe39%' THEN 1 ELSE 0 END AS SBE39,  ",
                  ...
                  "  GFBioSQL.dbo.FE_EVENT_SENSOR ON GFBioSQL.dbo.TRAP_SENSOR.FE_EVENT_SENSOR_CODE =  ",
                  " GFBioSQL.dbo.FE_EVENT_SENSOR.FE_EVENT_SENSOR_CODE INNER JOIN ",
                  " GFBioSQL.dbo.FISHING_EVENT AS FE ON GFBioSQL.dbo.TRAP_SENSOR.FISHING_EVENT_ID = FE.FISHING_EVENT_ID INNER JOIN ",
                  " dbo.SURVEY_trips_vw ON FE.TRIP_ID = dbo.SURVEY_trips_vw.TRIP_ID ",
                  " GROUP BY FE.TRIP_ID, FE.FE_MAJOR_LEVEL_ID, FE.FE_SUB_LEVEL_ID, FE.FE_MINOR_LEVEL_ID, dbo.SURVEY_trips_vw.YEAR, ",          
                  " GFBioSQL.dbo.FE_EVENT_SENSOR.FE_EVENT_SENSOR_DESC  ", 
                  " HAVING  (dbo.SURVEY_trips_vw.YEAR = 2021) AND (FE.FE_MAJOR_LEVEL_ID IN (7, 14, 27, 63)) ",
                  " ) AS SENS  GROUP BY YEAR, TRIP_ID, set_no, becket_no, trap_no", sep="")
    ...
    colnames(Trap.camera)  <- c( "Spatial Strata", "Set", "Becket id" , "Trap id", "SBE39", "HOBO", "CAM")
...
(ref:table5Caption) Summary of species captured by the `r boat` during the `r yr` survey standardized sets conducted at Dean/Burke Channel mainland inlet locality. No value in both weight and count fields indicate trace weights.
